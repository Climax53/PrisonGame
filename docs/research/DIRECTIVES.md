...
| 3 | **Failure fast, legible, interesting to recover from — telegraph danger the day before.** | ONI tedious rebuilds; Frostpunk invisible death-spirals. Desktop #6. | ✅ Honest next-day danger forecast bars (riot/fire/sickness/escape) sharing the event engine's own probabilities (`src/core/danger.ts`) |
